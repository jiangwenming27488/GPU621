...
#pragma omp single
			...
			//stage1Results[itile] = reduce(in + itile * tile_size, itile == last_tile ? last_tile_size : tile_size, combine, T(0));
#pragma omp barrier
//#pragma omp single
			/*int itile = omp_get_thread_num();
			int ntiles = omp_get_num_threads();
			...
			for (int i = itile; i < ntiles; i += ntiles) {
				stage1Results[i] = reduce(in + i * mtiles, i == last_tile ? last_tile_size : mtiles, combine, T(0));
			}*/
			...
			//scan_fn(in + itile * tile_size, out + itile * tile_size, itile == last_tile ? last_tile_size : tile_size, combine, stage2Results[itile]);




			/*for (int i = itile; i < ntiles; i += ntiles) {
				scan_fn(in + i * mtiles, out + i * mtiles, i == last_tile ? last_tile_size : mtiles, combine, stage2Results[i]);
			}*/
...
int main(int argc, char** argv) {
	...
	std::cout << "SPMD Prefix Scan" << std::endl;
